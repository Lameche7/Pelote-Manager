...
const [
  ...
  read(
    "../supabase/migrations/20260916144000_configure_championship_match_payments.sql",
  ),
  read(
  ...
  read(
    "../src/features/user-space/championships/services/championshipMatchReservationService.ts",
  ),
  read(
...
test("le paiement des rencontres est paramétrable par club", () => {
  assert.match(
    paymentMigration,
    /match_payment_mode text not null default 'free'/,
  );
...
test("le PCL peut réserver gratuitement sans contourner les contrôles de club", () => {
  ...
  assert.match(
    securityMigration,
    /federation_club\.linked_club_id = target_club_id/,
  );
});
