...
test(
  "une compétition est globale et les clubs abonnés sont liés séparément",
  async () => {
    const sql = await readMigration();
...
test(
  "l’identité sportive est globale par source et numéro de licence",
  async () => {
    const sql = await readMigration();
...
test(
  "les imports officiels gardent leur provenance et leurs types",
  async () => {
    const sql = await readMigration();
...
test(
  "le classement officiel est stocké sans imposer encore un moteur de calcul",
  async () => {
    const sql = await readMigration();
...
test(
  "les tables championnat ne sont pas exposées directement aux clients",
  async () => {
    const sql = await readMigration();
...
test(
  "l’administration du championnat passe par les droits du club",
  async () => {
    const sql = await readMigration();
