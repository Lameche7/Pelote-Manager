...
test(
  "PR124 trace le fichier externe sans stocker le PDF dans la base",
  async () => {
...
test(
  "PR124 conserve l identifiant equipe Errebot apres conversion native",
  async () => {
    const migration = await read(migrationPath);
...
test(
  "PR124 cree une identite externe reutilisable et distincte du joueur de tournoi",
  async () => {
    const migration = await read(migrationPath);
...
test(
  "le telephone est normalise mais ne vaut jamais preuve de compte a lui seul",
  async () => {
    const migration = await read(migrationPath);
