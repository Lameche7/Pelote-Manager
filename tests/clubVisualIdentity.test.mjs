...
test(
  "l’identité visuelle est stockée par club et validée côté serveur",
  async () => {
    ...
    assert.match(
      migration,
      /normalized_primary_color !~ '\^#\[0-9a-f\]\{6\}\$'/,
    );
...
test("les images sont téléversées dans un espace isolé par club", async () => {
  ...
  assert.match(service, /from\("club-branding"\)[\s\S]*\.upload/);
});

test(
  "l’accueil, le logo et le layout consomment la marque dynamique",
  async () => {
