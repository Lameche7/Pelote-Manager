...
test(
  "le tarif d'inscription est stocké en centimes et exposé au formulaire admin",
  async () => {
...
test(
  "les deux notifications de cycle utilisent le moteur central et sont idempotentes",
  async () => {
    const migration = await read(migrationPath);
