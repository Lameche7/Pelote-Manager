...
test(
  "les inscriptions créent des équipes privées, joueurs et disponibilités récurrentes",
  async () => {
...
test(
  "une équipe de pala contient exactement un Avant et un Arrière",
  async () => {
    const migration = await read(migrationPath);
...
test(
  "l'inscription utilisateur exige authentification, fenêtre ouverte et capacité",
  async () => {
    const migration = await read(migrationPath);
...
test(
  "la projection publique masque les coordonnées et ne montre que les équipes validées",
  async () => {
    const migration = await read(migrationPath);
    const start = migration.indexOf(
...
test(
  "l'administrateur peut gérer les équipes après clôture mais pas après les poules",
  async () => {
    const migration = await read(migrationPath);
    const start = migration.indexOf(
...
test(
  "les pages Tournois exposent consultation publique, inscription et gestion admin",
  async () => {
