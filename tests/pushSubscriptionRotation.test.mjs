...
test("la rotation d endpoint désactive uniquement l ancien endpoint connu de cet appareil", async () => {
...
test("le navigateur mémorise son endpoint précédent et le transmet atomiquement au backend", async () => {
  const service = await read(servicePath);

  assert.match(service, /pelote-manager:push:last-endpoint/);
  assert.match(service, /localStorage\.getItem\(LAST_PUSH_ENDPOINT_STORAGE_KEY\)/);
  assert.match(service, /localStorage\.setItem\(LAST_PUSH_ENDPOINT_STORAGE_KEY, endpoint\)/);
...
test("la désactivation volontaire oublie l endpoint local sans casser le multi-appareil", async () => {
  const service = await read(servicePath);
