...
begin
  ...
  if not exists (
    select 1
    from public.championship_reservation_windows as window
    where window.club_id = target_club_id
      and window.weekday = extract(isodow from local_start)::smallint
      and window.opens_at <= local_start::time
      and window.closes_at >= local_end::time
      and mod(
        extract(epoch from (local_start::time - window.opens_at))::bigint,
    ...
    'windows', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'opensAt', to_char(window.opens_at, 'HH24:MI'),
          'closesAt', to_char(window.closes_at, 'HH24:MI')
        )
        order by window.weekday
      )
      from public.championship_reservation_windows as window
      where window.club_id = target_club_id
    ), '[]'::jsonb)
...
as $$
  ...
  championship_periods as (
    ...
    join public.championship_reservation_windows as window
      on window.club_id = config.club_id
     and window.weekday = extract(isodow from day.calendar_date)::smallint
