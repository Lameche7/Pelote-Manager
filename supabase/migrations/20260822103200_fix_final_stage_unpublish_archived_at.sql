...
-- Le modèle Event Engine impose la cohérence suivante :
-- publication_status = 'archived' <=> archived_at is not null.
-- Le premier RPC de retrait des phases finales archivait l'événement sans
-- renseigner archived_at, ce qui faisait échouer la transaction sur la
-- contrainte de cohérence de public.events.
