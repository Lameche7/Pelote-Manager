alter function public.admin_preview_championship_standings_import(uuid, jsonb)
  rename to admin_preview_championship_standings_import_strict;

revoke all on function public.admin_preview_championship_standings_import_strict(uuid, jsonb)
  from public, anon, authenticated;

create function public.admin_preview_championship_standings_import(
  target_id uuid,
  payload jsonb
)
returns jsonb
    ...
    select team.id, team.source_label, team.pool_id,
      ...
      and not exists (
        ...
        where match.pool_id = team.pool_id
          and (match.team1_id = team.id or match.team2_id = team.id)
          and not (
            (match.team1_id = team.id and coalesce(match.score_raw, '') ~* '^Forfait général \\(Eq1\\)')
            or
            (match.team2_id = team.id and coalesce(match.score_raw, '') ~* '^Forfait général \\(Eq2\\)')
          )
...
revoke all on function public.admin_preview_championship_standings_import(uuid, jsonb)
  from public, anon, authenticated;
grant execute on function public.admin_preview_championship_standings_import(uuid, jsonb)
  to authenticated;
