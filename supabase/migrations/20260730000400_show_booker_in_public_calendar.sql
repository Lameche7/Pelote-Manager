-- Return only the public calendar projection, with a business title for every
-- occupation. Personal contact and licence data never leave this function.
drop function if exists public.list_calendar_occupations(uuid, timestamptz, timestamptz);
...
revoke all on function public.list_calendar_occupations(uuid, timestamptz, timestamptz) from public;
grant execute on function public.list_calendar_occupations(uuid, timestamptz, timestamptz) to anon, authenticated;

-- The weekly slot RPC powers the visible calendar cards. Keep its projection in
-- sync with list_calendar_occupations and use the exact same identity priority.
