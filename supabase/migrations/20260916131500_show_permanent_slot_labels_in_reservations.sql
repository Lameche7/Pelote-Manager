...
-- Les créneaux permanents restent visibles comme indisponibles dans Réservations,
-- mais leur libellé métier est affiché comme dans le Mode TV (ex. Ecole de Pelote).
-- Les autres occupations privées restent masquées et les occurrences libérées
-- conservent leur comportement réservable.
...
as $$
  ...
  permanent_slots_outside_schedule as (
    select
      ...
      permanent.permanent_label::text as booked_by_name,
      false as is_permanent_release,
  ...
  select
    ...
    case
      when slot.is_active_permanent_slot then
