...
export function AdminReservationsPage() {
  ...
  if (isLoading || !settings) {
    return <section className="admin-reservations"><p>Chargement…</p></section>;
  }
  ...
  return (
    <section className="admin-reservations" aria-labelledby="admin-reservations-title">
      <header>
        ...
        <p>Configurez les profils, délais, quotas, paiement, horaires et fermetures sans modifier le code.</p>
      </header>
      ...
      <div className="admin-reservations__panel">
        ...
        {!settings.onlinePaymentEnabled && (
          <p className="admin-reservations__alert" role="status">
            Paiement en ligne désactivé — les réservations sont confirmées immédiatement.
          </p>
        )}
        {settings.onlinePaymentEnabled && settings.paymentMode === "test" && (
          <p className="admin-reservations__alert" role="status">
            Paiement en ligne activé en MODE TEST — aucun paiement réel n’est effectué.
          </p>
        )}
        ...
        <div className="admin-reservations__form-grid">
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setSettings({ ...settings, onlinePaymentEnabled: event.target.value === "enabled" })
              }
            >
              <option value="disabled">Désactivé — réservation directe</option>
          ...
          {settings.onlinePaymentEnabled && (
            <label>
              ...
              <select
                ...
                onChange={(event) =>
                  setSettings({ ...settings, paymentMode: event.target.value as "test" | "helloasso" })
                }
              >
          ...
          <label>Anticipation non licencié (heures)<input type="number" min="0" value={settings.publicAdvanceHours} onChange={(event) => setSettings({ ...settings, publicAdvanceHours: Number(event.target.value) })} /></label>
          ...
          <label>Tarif non licencié (€)<input inputMode="decimal" value={centsToEuros(settings.publicPriceCents)} onChange={(event) => setSettings({ ...settings, publicPriceCents: eurosToCents(event.target.value) })} /></label>
          ...
          <label>Délai minimum avant réservation (minutes)<input type="number" min="0" value={settings.minimumNoticeMinutes} onChange={(event) => setSettings({ ...settings, minimumNoticeMinutes: Number(event.target.value) })} /></label>
          <label>Délai d’annulation (heures avant)<input type="number" min="0" value={settings.cancellationNoticeHours} onChange={(event) => setSettings({ ...settings, cancellationNoticeHours: Number(event.target.value) })} /></label>
          ...
          <label>Quota non licencié<input type="number" min="1" value={settings.publicMaxActiveReservations} onChange={(event) => setSettings({ ...settings, publicMaxActiveReservations: Number(event.target.value) })} /></label>
        </div>
      ...
      <div className="admin-reservations__panel">
        <label className="admin-reservations__resource">
          Terrain
          <select value={resourceId} onChange={(event) => void changeResource(event.target.value)}>
        ...
        <div className="admin-reservations__inline-form">
          <select aria-label="Jour" value={newHour.weekday} onChange={(event) => setNewHour({ ...newHour, weekday: Number(event.target.value) })}>
        ...
        <ul className="admin-reservations__list">
          {openingHours.map((hour) => (
            <li key={hour.id}>
              <span>{WEEKDAYS[hour.weekday - 1]} : {hour.opensAt.slice(0, 5)}–{hour.closesAt.slice(0, 5)}</span>
              <button type="button" onClick={() => void removeOpeningHour(hour.id)}>Supprimer</button>
            </li>
          ))}
        </ul>
        ...
        <ul className="admin-reservations__list">
          {closures.map((closure) => (
            <li key={closure.id}>
              <span>{closure.title} — {new Date(closure.startsAt).toLocaleString("fr-FR")} au {new Date(closure.endsAt).toLocaleString("fr-FR")}</span>
              <button type="button" onClick={() => void removeClosure(closure.id)}>Supprimer</button>
            </li>
          ))}
        </ul>
      </div>
