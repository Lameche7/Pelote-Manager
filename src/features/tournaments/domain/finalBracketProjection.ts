...
const winnerSourceLabel = (round: string, displayOrder: number) => {
...
export const buildFinalBracketProjectedMatches = ({
  ...
  const firstRoundSide = (
    ...
    return (
      ...
      unresolvedSide(winnerSourceLabel("preliminary", displayOrder))
    );
  };
  ...
  for (let roundIndex = 1; roundIndex < mainRounds.length; roundIndex += 1) {
    ...
    for (let displayOrder = 0; displayOrder < matchCount; displayOrder += 1) {
      ...
      projected.push({
        ...
        displayOrder,
        sideA:
          resolvedWinner(previousRound, previousA) ??
          unresolvedSide(winnerSourceLabel(previousRound, previousA)),
        sideB:
          resolvedWinner(previousRound, previousB) ??
          unresolvedSide(winnerSourceLabel(previousRound, previousB)),
      });
    }
  }
