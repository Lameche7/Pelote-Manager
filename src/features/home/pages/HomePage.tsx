...
const benefits = [
  ...
  { icon: "◉", title: "Infos en direct", text: "Retrouvez la vie des installations et les actualités du club." },
...
export function HomePage() {
  ...
  useEffect(() => {
    let active = true;
    const loadBranding = () => {
      clubBrandingService.getPublicBranding().then((value) => {
  ...
  useEffect(() => {
    if (isAuthLoading || !isAuthenticated) { setBanners([]); return; }
    let active = true;
    notificationService.listHomeBanners()
      .then((clubBanners) => { if (active) setBanners(clubBanners); })
    ...
    <div className="premium-home" style={heroStyle}>
      <section className="premium-home__hero" aria-labelledby="home-title">
        ...
        <div className="premium-home__content">
          <img className="premium-home__logo" src={branding.logoUrl} alt={branding.name} />
          <p className="premium-home__eyebrow">{branding.name}</p>
          <h1 id="home-title">Pelote Manager</h1>
          <p className="premium-home__signature">
            {CLUB_CONFIG.foundedYear && <>Depuis <strong>{CLUB_CONFIG.foundedYear}</strong> – </>}
            {CLUB_CONFIG.tagline}
          </p>
      ...
      {banners.length > 0 && (
        <section className="premium-home__announcements" aria-label="Informations importantes du club">
          <div className="premium-home__announcement-list">
            {banners.map((banner) => (
              <article className={`club-announcement club-announcement--${banner.priority}`} key={banner.communicationId}>
                <div><p>{bannerPriorityLabels[banner.priority]}</p><h2>{banner.title}</h2><span>{banner.body}</span></div>
      ...
      {showEventsSection && (
        <section className="premium-home__events" aria-labelledby="upcoming-events-title">
            ...
            {!eventsLoading && events.length > 0 && (
              <div className="premium-home__event-grid">
                {events.map((event) => {
                  ...
                  const eventLocation = [venueLabel, event.resourceNames.join(", ")].filter(Boolean).join(" · ");
                  return (
                    <article className="public-event-card" key={event.id} style={cardStyle}>
                      <div className="public-event-card__meta">
                        <span className="public-event-card__type">{event.typeName}</span>
                        {event.visibility === "members" && <span className="public-event-card__audience">Licenciés</span>}
                      </div>
                      <p className="public-event-card__period">{formatPublicEventPeriod(event.startsAt, event.endsAt)}</p>
                      <h3>{event.name}</h3>
                      {event.description && <p>{event.description}</p>}
                      {eventLocation && <p className="public-event-card__location">{eventLocation}</p>}
                    </article>
                  );
                })}
              </div>
      ...
      <section className="premium-home__club">
        <div className="premium-home__club-card">
          <img className="premium-home__club-logo" src={branding.logoUrl} alt={branding.name} />
          <div>
